var DOMtodo = {
    ...
    todoItemCloseVisible: 'todo-item__close-visible'
};
...
function addListItem(obj) {
  ...
  if (obj.isActive === true) {
    html = `<li class="todo-item" id="todo-${obj.id}">\
    <div class="todo-item__check js-todo-item__check"></div>\
    <label class="todo-item__label js-todo-item__label">${obj.text}</label>\
    <div class="todo-item__close js-todo-item__close"></div></li>`;
  }
  else {
    html = `<li class="todo-item todo-item_done" id="todo-${obj.id}">\
    <div class="todo-item__check js-todo-item__check"></div>\
    <label class="todo-item__label js-todo-item__label todo-item__label_done">${obj.text}</label>\
    <div class="todo-item__close js-todo-item__close"></div></li>`;
  }

  document.querySelector(`.${DOMtodo.todoList}`).insertAdjacentHTML('beforeend', html);
}
...
function visibleClose(id) {
  ...
  todoItem.querySelector(`.${DOMtodo.todoItemClose}`).classList.add(`${DOMtodo.todoItemCloseVisible}`);
}

function hideClose() {
  var allTodoItems = document.querySelectorAll('.todo-item');

  allTodoItems.forEach(function(current) {
    current.querySelector(`.${DOMtodo.todoItemClose}`).classList.remove(`${DOMtodo.todoItemCloseVisible}`);
  });
